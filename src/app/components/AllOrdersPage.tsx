...
import {
  ArrowLeft, ShoppingBag, Search, Filter, RotateCcw, TrendingUp, Trophy,
  Timer, Percent, IndianRupee, Layers,
} from "lucide-react";
import {
...
import { BulkOrder, BulkOrderCard } from "./ProductionPage";
...
export function AllOrdersPage({ onBack, superadmin = false }: { onBack?: () => void; superadmin?: boolean }) {
  ...
  // Orders are dated by their delivery deadline — the only date every order carries.
  ...
  // ── Analytics — all scoped to the same filtered set as the grid below ───────
  ...
  // Monthly commitment vs delivery, keyed on delivery deadline.
  ...
  // Deadline pipeline — nearest deadline first, with days remaining.
    ...
    <div style={{ minHeight: "100vh", background: T.silkCream, fontFamily: F.ui }}>
      ...
      <div style={{ padding: "28px 48px 0" }}>
        <div style={{ background: "#FFFFFF", borderRadius: 18, border: `1.5px solid ${T.borderDef}`, padding: "22px 24px", boxShadow: "0 4px 18px rgba(74,6,27,0.03)", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Timeline — scopes both the analytics and the order grid below */}
          <div>
