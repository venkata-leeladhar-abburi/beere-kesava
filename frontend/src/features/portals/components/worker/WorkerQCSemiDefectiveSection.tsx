...
function SemiDefectiveCard({ d, onView }: { d: DefectiveLogItem; onView: () => void }) {
  return (
    <div
      style={{
        background: "#FFFFFF",
    ...
    >
      <div style={{ padding: "14px 16px 12px", display: "flex", alignItems: "flex-start", gap: 10 }}>
        ...
        <span
          style={{
            ...
            fontFamily: F.u, fontSize: 12, fontWeight: 600, color: T.gold,
            background: T.bgGold, border: "1px solid rgba(200,155,71,0.30)",
            padding: "3px 9px", borderRadius: 999,
          }}
        >
          <RotateCcw size={13} /> Semi
        </span>
      </div>
      ...
      <div style={{ padding: "0 16px 12px" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {d.defects.length === 0 ? (
            ...
            <span key={df} style={{ fontFamily: F.u, fontSize: 12, fontWeight: 500, color: "#845E04", background: T.bgGold, border: "1px solid rgba(200,155,71,0.25)", padding: "3px 9px", borderRadius: 8 }}>
              {df}
            </span>
          ))}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 12, alignItems: "end", padding: "12px 16px", background: T.bg, borderTop: `1px solid ${T.bdr}` }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontFamily: F.u, fontSize: 18, fontWeight: 600, color: "#845E04", marginTop: 2, fontVariantNumeric: "tabular-nums", letterSpacing: "-0.01em" }}>
            {d.deduction}
          </div>
        </div>
      </div>
    ...
    <div id="wqc-semi-defective" style={{ margin: isDesktop ? "40px 0 0" : "32px 16px 0" }}>
      ...
      >
        <div style={{ marginBottom: 20 }}>
          <DateFilterBar filter={semiFilter} onChange={setSemiFilter} />
        </div>
        ...
        {filteredSemiLog.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: cols, gap: 16 }}>
            {filteredSemiLog.map((d) => (
              <SemiDefectiveCard key={d.id} d={d} onView={() => setViewing(d)} />
            ))}
          </div>
        )}
      </SectionCard>
