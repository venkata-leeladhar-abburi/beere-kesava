...
export function WorkerQCHistorySection({ items, historyFilter, setHistoryFilter, isDesktop, isTablet }: WorkerQCHistorySectionProps) {
  ...
  return (
    <>
      <div id="wqc-history" style={{ margin: isDesktop ? "20px 0 8px" : "20px 16px 8px", display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ width: 3, height: 18, background: T.green, borderRadius: 2 }} />
        <div style={{ fontFamily: F.u, fontSize: isDesktop ? 15 : 14, fontWeight: 700, color: T.brown }}>QC History</div>
      </div>
      <div style={{ margin: isDesktop ? "2px 0 10px" : "2px 16px 10px", fontFamily: F.u, fontSize: isDesktop ? 13 : 12, color: T.muted }}>
        All sarees that have passed quality check.
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: isDesktop ? "20px 0" : "20px 16px", textAlign: "center" }}>
          <div style={{ fontFamily: F.u, fontSize: 12, color: T.muted }}>No passed sarees in this range.</div>
        </div>
      ) : (
