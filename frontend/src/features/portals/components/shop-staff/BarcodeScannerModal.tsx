...
/**
 * Live camera barcode scanner used behind every "Open Camera" button in the
 * shop-staff flows. Decodes 1D saree-tag barcodes (Code128 by default, but
 * ZXing's MultiFormat reader also picks up QR/EAN/UPC if a tag ever uses
 * one) straight off the device camera feed — no manual typing needed once
 * the tag is in frame.
 */
      ...
      <div className="w-full max-w-[460px]" style={{ background: "#0F0906", borderRadius: 20, overflow: "hidden", boxShadow: "0 24px 60px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ padding: "12px 18px 16px" }}>
          <span style={{ fontFamily: F.u, fontSize: 12, color: "rgba(255,253,249,0.6)" }}>
            Hold the barcode tag steady inside the frame — it'll be picked up automatically.
          </span>
        </div>
      </div>
    </div>
