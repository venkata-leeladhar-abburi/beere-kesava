...
export function MaterialsFooter() {
  ...
  return (
    <footer style={{ background: T.silkCream, borderTop: `1px solid ${T.borderDef}`, marginTop: 64, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 0, right: 0, width: "40%", height: "100%", background: "radial-gradient(ellipse at top right, rgba(200,155,71,0.06), transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, width: "40%", height: "100%", background: "radial-gradient(ellipse at bottom left, rgba(110,15,45,0.04), transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ position: "relative", zIndex: 2, padding: `${isMobile ? 48 : 80}px ${px}px 32px`, display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 48 : 80, maxWidth: 1600, margin: "0 auto" }}>
        ...
        <div style={{ flex: "0 0 320px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 28 }}>
            <div style={{ width: 56, height: 56, borderRadius: 16, background: "rgba(200,155,71,0.12)", border: "1px solid rgba(200,155,71,0.35)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 12px 32px rgba(200,155,71,0.15)" }}>
              <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: 22, color: T.luxuryBrown, fontWeight: 500 }}>BK</span>
            </div>
            <div>
              <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 24, color: T.luxuryBrown, lineHeight: 1.1 }}>Beere Kesava</div>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {["f", "in", "yt", "li"].map((s) => (
              <motion.div key={s} whileHover={{ scale: 1.12, y: -2, backgroundColor: "rgba(200,155,71,0.15)", borderColor: "rgba(200,155,71,0.5)", color: T.luxuryBrown }} style={{ width: 40, height: 40, borderRadius: 12, border: "1px solid rgba(110,15,45,0.12)", background: "rgba(255,253,249,0.5)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.3s ease", color: T.taupe }}>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(auto-fit, minmax(160px, 1fr))", gap: isMobile ? 32 : 48 }}>
          {Object.entries(FOOTER_LINKS).map(([col, links]) => (
            <div key={col}>
              <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 18, color: T.luxuryBrown, marginBottom: 20 }}>{col}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {links.map(l => (
                  <motion.span key={l} whileHover={{ x: 4, color: T.antiqueGold }} transition={{ duration: 0.2 }} style={{ fontFamily: F.ui, fontWeight: 500, fontSize: 14, color: T.taupe, cursor: "pointer", display: "inline-block" }}>
                    {l}
                  </motion.span>
                ))}
              </div>
      ...
      <div style={{ position: "relative", zIndex: 2, margin: `0 auto`, maxWidth: 1600, padding: `24px ${px}px 32px`, display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 16 : 0, justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontFamily: F.mono, fontWeight: 500, fontSize: 12, color: T.luxuryBrown, letterSpacing: "2.5px", textTransform: "uppercase" }}>Tradition · Trust · Timeless Quality</span>
          <div style={{ width: 5, height: 5, borderRadius: "50%", background: T.antiqueGold, opacity: 0.7 }} />
        </div>
      </div>
    </footer>
