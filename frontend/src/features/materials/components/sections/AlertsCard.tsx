...
export function AlertsCard({ onCreatePO }: { onCreatePO?: () => void }) {
  ...
  const warpKg = stockItems.filter(i => i.materialType === "WARP").reduce((s, i) => s + Number(i.currentStock), 0);
  const reshamKg = stockItems.filter(i => i.materialType === "RESHAM").reduce((s, i) => s + Number(i.currentStock), 0);
  const jariBuns = stockItems.filter(i => i.materialType === "JARI").reduce((s, i) => s + Number(i.currentStock), 0);

  const dynamicAlerts = [
    {
      type: "WARP",
      current: `${Math.round(warpKg)} kg`,
      minLabel: `${thresholds.warp} kg`,
      isLow: warpKg < thresholds.warp,
      pct: Math.min(100, Math.round((warpKg / Math.max(1, thresholds.warp)) * 100)),
    },
    {
      type: "RESHAM",
      current: `${Math.round(reshamKg)} kg`,
      minLabel: `${thresholds.resham} kg`,
      isLow: reshamKg < thresholds.resham,
      pct: Math.min(100, Math.round((reshamKg / Math.max(1, thresholds.resham)) * 100)),
    },
    {
      type: "JARI",
      current: `${Math.round(jariBuns)} ${thresholds.jari.unit}`,
      minLabel: `${thresholds.jari.qty} ${thresholds.jari.unit}`,
      isLow: jariBuns < thresholds.jari.qty,
      pct: Math.min(100, Math.round((jariBuns / Math.max(1, thresholds.jari.qty)) * 100)),
    },
  ].filter(a => a.isLow);
        ...
        {dynamicAlerts.length === 0 ? (
          <div style={{ background: "rgba(30,102,64,0.06)", border: "1px solid rgba(30,102,64,0.18)", borderRadius: 10, padding: "16px 20px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontFamily: F.ui, fontSize: 14, fontWeight: 600, color: T.green }}>
              Stock levels healthy across Warp, Resham, and Jari. No immediate reorders required.
            </span>
          </div>
        ) : (
