...
export function RecentProcurementSection({ onViewAllPurchases }: { onViewAllPurchases: () => void }) {
  ...
  const allRecentItems = grnItems.flatMap(grn =>
    grn.items.map(item => ({
  ...
  return (
    <section id="mat-recent" style={{ padding: `44px ${px}px 0` }}>
      ...
      {allRecentItems.length === 0 ? (
        ...
        <div>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(4, 1fr)", gap: isMobile ? 16 : 22, alignItems: "stretch", marginBottom: 20 }}>
          ...
          <Pagination page={pag.page} pageCount={pag.pageCount} total={pag.total} pageSize={pag.pageSize} start={pag.start} onPageChange={pag.setPage} onPageSizeChange={pag.setPageSize} itemLabel="deliveries" />
        </div>
      )}
