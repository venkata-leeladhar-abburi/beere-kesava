...
export function CashFlowTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{ background: "#FFFDF9", border: `1px solid ${T.borderDef}`, borderRadius: 9, padding: "10px 14px", boxShadow: "0 4px 16px rgba(74,6,27,0.12)" }}>
      <div style={{ fontVariantNumeric: "tabular-nums", fontSize: 12, color: T.taupe, marginBottom: 5, textTransform: "uppercase" }}>{label}</div>
      {payload.map((p: any) => (
        <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: T.luxuryBrown }}><Money value={rupees(Number(p.value))} /></span>
        </div>
      ))}
    </div>
  );
