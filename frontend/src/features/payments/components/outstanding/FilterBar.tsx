        ...
        <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {(["all", ...AGE_BUCKETS] as AgeKey[]).map(k => (
            <button key={k} onClick={() => setAgeFilter(k)}
              style={{ fontFamily: F.ui, fontSize: 12, fontWeight: 600, padding: "8px 14px", borderRadius: 99, cursor: "pointer", background: ageFilter === k ? T.royalBurgundy : "transparent", color: ageFilter === k ? "#FFFDF9" : T.taupe, border: ageFilter === k ? "none" : `1.5px solid rgba(110,15,45,0.18)` }}>
              {k === "all" ? "All ages" : `${k} d`}
            </button>
          ))}
        </div>
