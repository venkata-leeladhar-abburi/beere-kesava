    ...
    <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: 16 }}>
      {[
        ...
        <div key={f.label} style={{ ...card, padding: "20px 22px" }}>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.2px", color: T.taupe, marginBottom: 8 }}>{f.label}</div>
      ...
      <div style={{ gridColumn: "1 / -1", ...card, padding: "20px 22px" }}>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.2px", color: T.taupe, marginBottom: 8 }}>Address</div>
        <div style={{ fontFamily: F.ui, fontSize: 14, color: T.luxuryBrown, lineHeight: 1.65 }}>{supplier.address || "—"}</div>
      </div>
      {supplier.notes && (
        <div style={{ gridColumn: "1 / -1", ...card, padding: "20px 22px" }}>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.2px", color: T.taupe, marginBottom: 8 }}>Notes</div>
          <div style={{ fontFamily: F.ui, fontSize: 14, color: T.luxuryBrown, lineHeight: 1.65 }}>{supplier.notes}</div>
        </div>
      )}
      {supplier.visitingCard && (
        <div style={{ gridColumn: "1 / -1", ...card, padding: "20px 22px" }}>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.2px", color: T.taupe, marginBottom: 10 }}>Visiting Card</div>
          <img src={supplier.visitingCard} alt="Visiting card" className="max-w-[380px] w-full" style={{ borderRadius: 10, border: `1px solid ${T.borderDef}` }} />
        </div>
      )}
    </div>
  );
